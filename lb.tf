...
resource "aws_security_group_rule" "allow_http_from_alb" {
  ...
  security_group_id        = module.glpi.glpi_ec2_security_group_id
...
resource "aws_lb_target_group_attachment" "glpi_tg_attachment" {
  ...
  target_id        = module.glpi.glpi_instance_id
  port             = 80
}
...
# Create self-signed certificate using ALB DNS name
resource "tls_self_signed_cert" "alb_cert" {
  ...
  # Add Subject Alternative Names (SANs) for flexibility
  dns_names = [
